...
cmd_graph_search() {
    ...
    shift
    local depth="1" edges="" limit="10" project=""
    ...
    # Get initial results using FTS5
    local initial_results
    initial_results=$(search_query "$query" "$project" "$limit")
...
main() {
    [[ $# -eq 0 ]] && {
        ...
        echo ""
        echo "Commands:"
        echo "  build [--no-graph]          Build/rebuild the search index"
